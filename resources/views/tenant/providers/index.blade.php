        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <a href="{{ route('tenant.providers.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$provider->id}}</td>
                            <td id="provider_{{$provider->id}}">{{$provider->name_company}}</td>
                            <td>{{$provider->provider_representant}}</td>
                            <td>{{$provider->phone}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$provider->id}}">
                                ...
                                <button onclick="onOpenModal({{$provider->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#provider_' + id).text();

        const text = `Realmente deseja excluir o fornecedor ${name} ?`;
