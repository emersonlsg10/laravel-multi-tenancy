        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    <h4 class="card-title">Lista de Brindes</h4>
                    <a href="{{ route('tenant.gifts.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$gift->id}}</td>
                            <td id="gift_{{$gift->id}}">{{$gift->name}}</td>
                            <td>{{$gift->cost}}</td>
                            <td>{{$gift->value}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$gift->id}}">
                                ...
                                <button onclick="onOpenModal({{$gift->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#gift_' + id).text();

        const text = `Realmente deseja excluir o brinde ${name} ?`;
