        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    <h4 class="card-title">Lista de Caixa</h4>
                    <a href="{{ route('tenant.cashiers.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$cashier->id}}</td>
                            <td id="cashier_{{$cashier->id}}">{{$cashier->name_cashier}}</td>
                            <td>{{$cashier->class}}</td>
                            <td>{{$cashier->type}}</td>
                            <td>{{$cashier->setor}}</td>
                            <td>{{$cashier->name}}</td>
                            ...
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$cashier->id}}">
                                ...
                                <button onclick="onOpenModal({{$cashier->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#cashier_' + id).text();

        const text = `Realmente deseja excluir o caixa ${name} ?`;
