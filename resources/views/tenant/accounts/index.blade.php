        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    <h4 class="card-title">Lista de Contas a pagar</h4>
                    <a href="{{ route('tenant.accounts.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$account->id}}</td>
                            <td><?= $account->status == 1 ? 'Aberta' : 'Paga' ?></td>
                            <td id="account_{{$account->id}}">{{$account->name}}</td>
                            <td>{{$account->due_date}}</td>
                            <td>{{$account->value}}</td>
                            <td>{{$account->observation}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$account->id}}">
                                ...
                                <button onclick="onOpenModal({{$account->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
