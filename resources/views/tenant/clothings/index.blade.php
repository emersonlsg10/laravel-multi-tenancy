        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    <h4 class="card-title">Lista de Vestuário</h4>
                    <a href="{{ route('tenant.clothings.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$clothing->id}}</td>
                            <td id="clothing_{{$clothing->id}}">{{$clothing->name}}</td>
                            <td>{{$clothing->model}}</td>
                            <td>{{$clothing->cost}}</td>
                            <td>{{$clothing->value}}</td>
                            <td>{{$clothing->color}}</td>
                            <td>{{$clothing->quantity}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$clothing->id}}">
                                ...
                                <button onclick="onOpenModal({{$clothing->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#clothing_' + id).text();

        const text = `Realmente deseja excluir o vestuário ${name} ?`;
