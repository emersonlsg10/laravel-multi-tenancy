        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <a href="{{ route('tenant.materials.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$material->id}}</td>
                            <td id="material_{{$material->id}}">{{$material->type}}</td>
                            <td>{{$material->category}}</td>
                            <td>{{$material->color}}</td>
                            <td>{{$material->measure}}</td>
                            <td>{{$material->quantity}}</td>
                            <td>{{$material->size}}</td>
                            <td>{{$material->name_company}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$material->id}}">
                                ...
                                <button onclick="onOpenModal({{$material->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#material_' + id).text();

        const text = `Realmente deseja excluir o material ${name} ?`;
