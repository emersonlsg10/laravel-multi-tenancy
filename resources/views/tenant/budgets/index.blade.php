        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <a href="{{ route('tenant.budgets.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$budget->id}}</td>
                            <td id="budget_{{$budget->id}}">{{$budget->model}}</td>
                            <td>{{$budget->budget_type}}</td>
                            <td>{{$budget->value}}</td>
                            <td>{{$budget->created_at}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$budget->id}}">
                                ...
                                <button onclick="onOpenModal({{$budget->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#budget_' + id).text();

        const text = `Realmente deseja excluir o orçamento ${name} ?`;
