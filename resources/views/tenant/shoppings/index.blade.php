        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <a href="{{ route('tenant.shoppings.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$shopping->id}}</td>
                            <td id="shopping_{{$shopping->id}}">{{$shopping->name_shopping}}</td>
                            <td>{{$shopping->value}}</td>
                            <td>{{$shopping->quantity}}</td>
                            <td>{{$shopping->payment}}</td>
                            <td>{{$shopping->invoice_date}}</td>
                            <td>{{$shopping->observation}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$shopping->id}}">
                                ...
                                <button onclick="onOpenModal({{$shopping->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#shopping_' + id).text();

        const text = `Realmente deseja excluir o item ${name} ?`;
