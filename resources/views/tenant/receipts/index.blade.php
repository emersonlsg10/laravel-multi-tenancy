        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    <h4 class="card-title">Lista de Contas a Receber</h4>
                    <a href="{{ route('tenant.receipts.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$receipt->id}}</td>
                            <td><?= $receipt->status == 1 ? 'Aberta' : 'Paga' ?></td>
                            <td id="receipt_{{$receipt->id}}">{{$receipt->name}}</td>
                            <td>{{$receipt->due_date}}</td>
                            <td>{{$receipt->value}}</td>
                            <td>{{$receipt->observation}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$receipt->id}}">
                                ...
                                <button onclick="onOpenModal({{$receipt->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#receipt_' + id).text();

        const text = `Realmente deseja excluir a conta em nome de ${name} ?`;
