        ...
        <div class="card">
            <div class="card-body">
                <div style="margin-bottom: 30px;display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <a href="{{ route('tenant.stocks.create', ['prefix' => \Request::route('prefix')]) }}">
                ...
                <table id="datatable" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$stock->id}}</td>
                            <td id="stock_{{$stock->type}}">{{$stock->type}}</td>
                            <td>{{$stock->quantity}}</td>
                            <td>{{$stock->payment_form}}</td>
                            <td>{{$stock->unit_value}}</td>
                            <td>{{$stock->stock_value}}</td>
                            <td style="display: flex; flex-wrap: nowrap;justify-content: space-between">
                                <a style="text-decoration: none" href="{{ url()->current().'/editar/'.$stock->id}}">
                                ...
                                <button onclick="onOpenModal({{$stock->id}})" type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal">
...
<script>
    function onOpenModal(id) {
        ...
        const name = $('#stock_' + id).text();

        const text = `Realmente deseja excluir o stock ${name} ?`;
